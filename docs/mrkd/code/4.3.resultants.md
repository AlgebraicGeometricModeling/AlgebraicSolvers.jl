...
## Projective resultants

Projective resultant matrices are constructed as coefficient matrices of the monomial multiples of the input polynomials `P=[P1, P2, ...]` of total degree bounded by $\rho$: 

```math
[\mathcal{M}_{\rho -\deg(P_1)} P_1, \ldots, \mathcal{M}_{\rho -\deg(P_n)} P_n]
```
where $\mathcal{M}_d$ is the set of monomials of degree $\le d$. For Macaulay constructions, $\rho= \sum_i \deg(P_i)-n+1$, where $n$ is the number of variables.
